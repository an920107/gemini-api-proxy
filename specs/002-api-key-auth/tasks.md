...
*   **Tasks**:
    - [X] T001 Add `sha2` and `hex` dependencies to `Cargo.toml`.
...
*   **Tasks**:
    - [X] T002 Create a new SQLx migration file for the `api_keys` table using `sqlx migrate add create_api_keys_table`.
    - [X] T003 [P] Define the `api_keys` table schema, index, and seed data in the new migration file in `migrations/`.
    - [X] T004 [P] Create a new utility module and `hash_api_key` function in `src/utils/crypto.rs`.
    - [X] T005 [P] Create the `/v1beta/models` endpoint module and a placeholder handler in `src/routes/models.rs`.
...
*   **Tasks**:
    - [X] T006 [US1] Create the integration test file at `tests/auth_check.rs`.
    - [X] T007 [US1] Write a failing integration test for the valid key scenario (GET /v1beta/models) in `tests/auth_check.rs`.
    - [X] T008 [US1] Create the authentication middleware module `src/middleware/auth.rs`.
    - [X] T009 [US1] Implement the middleware logic to extract the `x-goog-api-key` header.
    - [X] T010 [US1] Implement the database query logic within the middleware to find the corresponding `hashed_key`.
    - [X] T011 [US1] Register the `/v1beta/models` route and apply the `ApiKeyAuth` middleware in `src/main.rs`.
...
*   **Tasks**:
    - [X] T012 [US2] Write an integration test for the invalid key scenario in `tests/auth_check.rs`.
...
*   **Tasks**:
    - [X] T013 [US3] Write an integration test for the missing key scenario in `tests/auth_check.rs`.

### Phase 6: Polish & Verification

*   **Goal**: Ensure code quality, run all checks, and finalize the feature.
*   **Tasks**:
    - [X] T014 [P] Run `cargo fmt` to format the codebase.
    - [X] T015 [P] Run `cargo clippy` to check for and fix lints.
    - [X] T016 Run all tests with `cargo test` to ensure all scenarios pass and there are no regressions.
